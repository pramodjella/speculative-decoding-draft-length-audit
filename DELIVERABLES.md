...
1. **The per-step signal lives in the draft hidden state — not in cheap features, not in
   position.** Cheap logit features fail (E2: −2.7–8.5%); static position signal degenerates
...
> within-fold** rerun: `analyze_bayes_ceiling_paired.py` -> `bayes_ceiling_paired.json`, in
> which the ceiling genuinely bounds the deployable probe on the same test rows. Numbers in
> this section (+2.4-3.4% net, +18.3%/+4.3%) are the superseded un-paired values and are kept
> for the audit trail only -- do not quote them. The paired values are +3.1/+2.0/+2.5% net.
>
> **Span convention, for anyone reconciling Table I by hand.** Recovery is computed per fold
> and averaged, so each fold's recovery is a fraction of *its own* oracle span. The paper's
> "perfect threshold choice adds between 1.0 and 3.2% of the span" follows the same per-fold
> convention. The `threshold_selection_loss_pct_of_span` field in `bayes_ceiling_paired.json`
> instead divides by the whole-capture span and therefore reads 0.87-3.11. Both are correct;
> they differ only in the denominator, and the two conventions shift the recovery columns by
> at most 1.6 pp.

Artifacts: [results/perstep_signal/bayes_ceiling.md](results/perstep_signal/bayes_ceiling.md)
(+ .json), [results/perstep_signal/bayes_ceiling_paired.md](results/perstep_signal/bayes_ceiling_paired.md)
(+ .json -- **the paper's source**),
[results/perstep_signal/bayes_ceiling_control.md](results/perstep_signal/bayes_ceiling_control.md),
...
Reproduce: `python analyze_bayes_ceiling_paired.py` (paper), `python analyze_bayes_ceiling.py`
then `python analyze_bayes_ceiling_control.py` (historical + permutation control).
