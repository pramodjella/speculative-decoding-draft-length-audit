% Figures are PNG, so this must build under pdflatex. arXiv's AutoTeX picks the engine
% itself unless told; declaring it in the first lines is their documented way to be sure.
\pdfoutput=1
...
\author{
% No titles in the byline, per ML venue convention. All six authors share one affiliation, so a
% single unmarked line is correct and no \IEEEauthorrefmark superscripts are needed.
...
hidden state does carry the signal, at AUC $0.84$ to $0.88$. Gate three puts that signal in a
running engine, where acting on it costs $2$ to $17\%$ against a tuned fixed depth. One policy
survives the audit: signal-free saturation tail-pruning, worth $+2$ to $6\%$ at batch size~1,
...
recover $-2.7$ to $8.5\%$ of the span and go negative on the reasoning head, nor in token
position, which recovers exactly zero. It is
...
probability saturates. It is worth $+2$ to $6\%$ at batch size 1, and the mechanism explains
...
\cite{zhang2024svip,adaedl,specdec,ddd}.
...
exact, so decoding is lossless by construction: the draft length changes what is proposed,
never what is emitted. Every comparison here rests on that property rather than on a
token-identity audit of these runs, which we did not carry out. The wall-clock experiments use
the SafeAILab/EAGLE-3 reference implementation because vLLM exposes no per-step draft-length
hook.
...
and averaged. The two conventions for the span are separated here: the recovery columns divide
by a per-fold span, while the Oracle span column is the pooled whole-capture figure, which runs
up to $0.6$\,pp above the per-fold mean. Dividing the recovery columns by the pooled span
instead shifts them by at most $1.6$\,pp and changes no conclusion. Bayes(position) is omitted because it is $+0.0\%$ in all five settings by
...
the span. Perfect threshold choice would add between $1.0$ and $3.2\%$ of the span, and we
...
recovery and $+2.9\%$ net. \emph{Detection:} the probe we report reads AUC $0.84$ to $0.88$
across the three models, and the weaker probe families of \S\ref{sec:gate2} run down to
$0.79$, against $0.484$ for an earlier 16-dimensional random projection. The weak probe was the earlier limitation,
...
against 2.40 on the same task, position-0 acceptance 0.28 against 0.55. Detection is not what
fails here: the probe reads this head at AUC $0.87$, as well as it reads either instruction
head. What a weak head yields is a small ceiling, and a fifth of a $+4.5\%$ span is nothing
worth collecting. That is the same detection-versus-exploitation gap the rest of the audit
turns on, and head strength produces it whatever the model is doing. We therefore scope the
...
look. Entropy, margin and SVIP-style statistics recover between $-2.7$ and $8.5\%$ of the
span across the same three models, and the reasoning head is the negative one.
...
rejection at AUC $0.84$ and $0.88$.
...
$0.79$ and $0.88$, and the MLP does not beat PCA-50 with logistic regression on recovery,
which is the probe we report throughout. Capacity was not the missing ingredient.
...
\subsection{Three published-style policies}
\label{sec:zoo}
We extended the test to three stopping policies through the same verified codepath:
SVIP-style entropy, SADDLE/PACER-style cumulative path probability, and a SpecDec++-style
trained hidden probe. A margin-threshold arm ran in an earlier pass and was dropped before the
held-out verification, so we do not report a number for it.
...
fixed depth in the same run, mean over 3 repeats. Per-cell SE runs $0.4$ to $1.7$\,pp.
Every cell clears its own error bar except the two nominal ties, saturation tail-pruning on
HumanEval held-out ($+0.1\pm0.9$) and entropy on MT-Bench in-distribution ($+1.1\pm1.7$).
The two halves are the same policies, the
...
probability the drafter has already computed falls below a threshold. We write the rule as
cum($\tau$): stop once that running product drops below $\tau$, so a larger $\tau$ stops
sooner and drafts less. Each engine gets its own $\tau$ chosen on train, cum(0.05) in the
EAGLE-3 tree and cum(0.2) in the vLLM chain, and we name the value wherever we quote a number.
Zero model cost, no
...
It wins at a realized mean depth of 5.62 against fixed depths of 5 to 8. Outputs are identical, the
...
$+1.3\%$, $+0.3\%$ and $+0.7\%$ at mean draft 3.05. The gentler cum(0.05), which drafts to a
mean of 4.22 there, loses instead: $-4.3\%$, $-7.3\%$ and $-4.9\%$.
...
worst is a tie. Each cell is that container's own paired mean over 4 cycles; the last column
is the mean over the three containers with its standard error, the spread across containers
divided by $\sqrt{3}$.}
\label{tab:fairbase}
\centering
\small
...
\midrule
HumanEval & $+6.9$ & $+6.6$ & $+3.3$ & $+5.6{\pm}1.2$ \\
GSM8K & $+7.1$ & $+8.4$ & $+1.8$ & $+5.7{\pm}2.0$ \\
MT-Bench & $+3.5$ & $-0.2$ & $+2.7$ & $+2.0{\pm}1.1$ \\
\bottomrule
\end{tabular}
\end{table}
...
for Table~\ref{tab:fairbase} is therefore $+2$ to $6\%$ with one of three workloads a wash:
MT-Bench reads $+2.0\pm1.1$ across containers, which three containers cannot separate from
zero, its worst cell reads $-0.2\pm2.0$ within its own run, and no cell is a significant
loss.
...
$B{=}8$. Realized depth moves 2.30, 2.68, 2.57 across the three batch sizes, rising where
the gain first erodes rather than tracking it all the way down. We use a batch-level
...
policy at its train-selected $\tau$ ties or beats the tuned fixed baseline everywhere we
...
savings can only shrink. The $+2$ to $6\%$ is a $B{=}1$ ceiling.
...
\emph{Reconciling prior gains.} SVIP \cite{zhang2024svip}, AdaEDL \cite{adaedl}, SpecDec++
\cite{specdec} and DDD \cite{ddd} report 2 to 8\% against
...
measurements from this setting explain the pattern. First, the prize is capped: the
...
barely persists. The signal capture behind this harness covers 1{,}295 decode steps. Across
them the accept-run distribution is heavily zero-inflated, with $61\%$ of steps accepting no
draft token at all, and the lag-1 autocorrelation of the accept-run is only $0.247$. What the
last step taught is a weak guide to the next one, so an online learner spends its budget
tracking something close to noise.
...
spread can exceed the within-run bars: MT-Bench reads $+3.5\pm0.3$ on one container and
$-0.2\pm2.0$ on another, a gap wider than either run could see from inside itself. That is the
honest statement of how far these magnitudes should be trusted.
\end{itemize}
...
acts on it, because verification dominates execution. We audited three published-style
stopping policies on the clock, and re-ran the one that survived under a paired,
drift-controlled protocol that six evaluation failure modes forced on us. Exactly one policy is left standing: signal-free saturation tail-pruning,
worth $+2$ to $6\%$ at zero model cost, with one of three workloads a wash and only at low
batch. The deployable recommendation
...
% A list, not a paragraph: the run-in version packed long \texttt{} filenames into a narrow
% column and produced a dozen badly stretched lines, and a list of scripts reads better as a
% list anyway.
Everything is released at
\url{https://github.com/pramodjella/speculative-decoding-draft-length-audit}, including the raw
per-run JSON behind every table and figure here except Table~\ref{tab:inloop}, whose harness
writes to a Modal volume; its cells are recorded in \texttt{DELIVERABLES.md}, which maps each
result (E1--E6) to the script and artifact that produced it.

\begingroup
\small
\begin{itemize}\itemsep1pt \parskip0pt
\item Hidden-state capture: \texttt{modal\_eagle3\_hidden\_\allowbreak full\_capture.py}
\item Decomposition: \texttt{analyze\_bayes\_\allowbreak ceiling\_paired.py}, the
paired-within-fold version carrying the nesting and position self-tests, with its permutation
control and independent re-derivation
\item Cost grounding: \texttt{measured\_cost\_ground.py}
\item Wall-clock harnesses: \texttt{modal\_eagle\_\allowbreak wallclock.py},
\texttt{modal\_eagle\_inloop.py}
\item Policy zoo and paired protocol: \texttt{modal\_eagle\_\allowbreak zoo\_verify.py},
\texttt{modal\_eagle\_paired.py}
\item Fair-baseline native-engine replication: \texttt{modal\_vllm\_fairbase.py}
\item Per-request controllers (Table~\ref{tab:bandit}):
\texttt{modal\_controller\_\allowbreak benchmark.py}
\end{itemize}
\endgroup

\noindent A single \texttt{scripts/check\_stale.sh} gate guards the shipped documents against
retired numbers.
