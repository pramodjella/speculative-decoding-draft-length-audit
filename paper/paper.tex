...
and averaged. Oracle span is computed per fold rather than once over the whole capture;
the whole-capture convention shifts the recovery columns by at most $1.6$\,pp and
changes no conclusion. Bayes(position) is omitted because it is $+0.0\%$ in all five settings by
