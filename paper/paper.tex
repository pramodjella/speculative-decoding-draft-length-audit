...
forwards, so a $K$-token step costs $1{+}CK$. The saturation policy of \S\ref{sec:survivor} is
written cum($\tau$): stop drafting once the running best-path probability falls below $\tau$,
so a larger $\tau$ stops sooner. Each engine gets its own $\tau$ chosen on train, cum(0.05) in
the EAGLE-3 tree and cum(0.2) in the vLLM chain.
...
trained hidden probe. A margin-threshold arm ran in the first, in-distribution pass and did
not carry into the held-out reruns, so there is no held-out number for it.
...
MT-Bench reads $+2.0\pm1.1$ over containers, and three containers cannot separate that from
zero. Its worst cell is $-0.2\pm2.0$. No cell is a significant loss.
...
$B{=}8$. Realized depth moves 2.30, 2.68, 2.57 across the three batch sizes. We use a
batch-level
...
depth rising at $B{=}4$ is that compromise becoming visible.
...
\url{https://github.com/pramodjella/speculative-decoding-draft-length-audit}.
\texttt{DELIVERABLES.md} maps each result (E1--E6) to the script and artifact that produced
it. The raw per-run JSON behind every table and figure here is released with it. The one
exception is Table~\ref{tab:inloop}, whose harness writes to a Modal volume; its cells are
recorded in \texttt{DELIVERABLES.md}.
